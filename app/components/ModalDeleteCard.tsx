...
import { saveLocalStorageComponents } from "../hooks/funtions";

interface Props {
    id: number
}

const ModalDeleteCard: React.FC<Props> = ({id}) => {
    const {setModalDelete, componentsCity, setComponentsCity} = useMyContext()

    const deleteCity = ()  => {
        const newComponents = [...componentsCity]
        newComponents[id] = 'AddCard'
  ...
  return (
    <div className="fixed top-0 w-full h-full flex items-center justify-center bg-black-clear overflow-hidden">
        <div className="group select-none w-[250px] flex flex-col p-4 relative items-center justify-center bg-gray-800 border border-gray-800 shadow-lg rounded-2xl">
            ...
            <div className="p-2 mt-2 text-center space-x-1 md:block">
              ...
              <button className="bg-red-500 hover:bg-transparent px-5 ml-4 py-2 text-sm shadow-sm hover:shadow-lg font-medium tracking-wider border-2 border-red-500 hover:border-red-500 text-white hover:text-red-500 rounded-full transition ease-in duration-300" onClick={deleteCity}>
                Confirm
              </button>
            </div>
        </div>
